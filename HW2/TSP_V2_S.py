...
def plot_graph_fitness_iterations(title, array1, array2, array3, array4):
    ...
    plt.savefig('images/TSP_Fit_Small.png')
    plt.show()
...
def plot_bar_max_fitness(title, array1, array2, array3, array4):
    ...
    plt.legend()
    plt.savefig('images/TSP_Bar_Small.png')
    plt.show()
...
def plot_bar_max_fitness_iteration(title, array1, array2, array3, array4):
    ...
    plt.ylabel('Max Fitness')
    plt.legend()
    plt.savefig('images/TSP_MaxIteration_Small.png')
    plt.show()
...
def plot_graph_fevals_iterations(title, array1, array2, array3, array4):
    ...
    plt.savefig('images/TSP_FEvals_Small.png')
    plt.show()
...
def plot_clock_time_table(title, array1, array2, array3, array4):
    ...
    table.scale(1.2, 1.2)
    plt.savefig('images/TSP_ClockTime_Small.png')
    plt.show()
...
def test_tsp():
    ...
    problem = TSPGenerator.generate(seed=7, number_of_cities=10, area_width=10, area_height=10)
    ...
    fevals_rhc = df_run_curves_rhc['FEvals'].iloc[3401:3600].to_numpy()
    fit_rhc = (df_run_curves_rhc['Fitness'].iloc[3401:3600] * -1).to_numpy()
    clock_time_rhc = sum(df_run_curves_rhc['Time'].iloc[3401:3600].to_numpy())
    ...
    """
    plot_graph_fitness_iterations('Fitness vs Iteration: TSP using RHC vs SA vs GA vs MIMIC-Small', array1=fit_rhc,
                                  array2=fit_sa, array3=fit_ga, array4=fit_mimic)
    plot_bar_max_fitness('Max Fitness: TSP using RHC vs SA vs GA vs MIMIC-Small', array1=max(fit_rhc),
                         array2=max(fit_sa), array3=max(fit_ga), array4=max(fit_mimic))
    plot_bar_max_fitness_iteration('Max Fitness Iteration: TSP using RHC vs SA vs GA vs MIMIC-Small',
                                   array1=fit_rhc,
                                   array2=fit_sa, array3=fit_ga, array4=fit_mimic)
    plot_graph_fevals_iterations('FEvals vs Iteration: TSP using RHC vs SA vs GA vs MIMIC-Small', array1=fevals_rhc,
                                  array2=fevals_sa, array3=fevals_ga, array4=fevals_mimic)
    plot_clock_time_table('Clock Time: TSP using RHC vs SA vs GA vs MIMIC-Small', array1=clock_time_rhc,
