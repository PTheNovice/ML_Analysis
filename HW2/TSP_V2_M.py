...
def plot_graph_fitness_iterations(title, array1, array2, array3, array4):
    ...
    plt.savefig('images/TSP_Fit_Medium.png')
    plt.show()
...
def plot_bar_max_fitness(title, array1, array2, array3, array4):
    ...
    plt.legend()
    plt.savefig('images/TSP_Bar_Medium.png')
    plt.show()
...
def plot_bar_max_fitness_iteration(title, array1, array2, array3, array4):
    ...
    plt.ylabel('Max Fitness')
    plt.legend()
    plt.savefig('images/TSP_MaxIteration_Medium.png')
    plt.show()
...
def plot_graph_fevals_iterations(title, array1, array2, array3, array4):
    ...
    plt.savefig('images/TSP_FEvals_Medium.png')
    plt.show()
...
def plot_clock_time_table(title, array1, array2, array3, array4):
    ...
    table.scale(1.2, 1.2)
    plt.savefig('images/TSP_ClockTime_Medium.png')
    plt.show()
...
def test_tsp():
    ...
    problem = TSPGenerator.generate(seed=7, number_of_cities=20, area_width=10, area_height=10)
    ...
    rhc = RHCRunner(problem=problem, experiment_name='RHC', output_directory='images/RHC.csv', seed=1,
                    iteration_list=[200], max_attempts=200, restart_list=[20])
    ...
    # print(df_run_curves_rhc)
    df_run_curves_rhc[['Fitness', 'FEvals', 'Time']] = df_run_curves_rhc[['Fitness', 'FEvals', 'Time']] / 5
    # df_run_curves_rhc = df_run_curves_rhc/10
    # print(df_run_curves_rhc)
    # fevals_rhc = df_run_curves_rhc['FEvals'].to_numpy()
    # fit_rhc = (df_run_curves_rhc['Fitness'] * -1).to_numpy()
    # clock_time_rhc = sum(df_run_curves_rhc['Time'].to_numpy())
    # Best run is the 7th run, starting from 0
    fevals_rhc = df_run_curves_rhc['FEvals'].iloc[1401:1600].to_numpy()
    fit_rhc = (df_run_curves_rhc['Fitness'].iloc[1401:1600] * -1).to_numpy()
    clock_time_rhc = sum(df_run_curves_rhc['Time'].iloc[1401:1600].to_numpy())

    """
    Simulated Annealing (SA) Algo - Small, Need Med, Large
    """
    init_state_q = [.5]  # For SA, if the temperature list is increased, it will run iterations based on this value
    sa = SARunner(problem=problem, experiment_name='SA', output_directory=None, seed=1,
    ...
    for i in range(100, 1000, 100):
        sa = SARunner(problem=problem, experiment_name='SA', output_directory=None, seed=i,
                      iteration_list=[200], max_attempts=200, temperature_list=init_state_q,
                      decay_list=[ExpDecay])
    ...
    df_run_curves_sa[['Fitness', 'FEvals', 'Time']] = df_run_curves_sa[['Fitness', 'FEvals', 'Time']] / 5
    print(df_run_curves_sa)
    fevals_sa = df_run_curves_sa['FEvals'].to_numpy()
    fit_sa = (df_run_curves_sa['Fitness'] * -1).to_numpy()
    clock_time_sa = sum(df_run_curves_sa['Time'].to_numpy())

    """
    Genetic Algorithm (GA) - Small, Need Med, Large
    """
    ga = GARunner(problem=problem, experiment_name='GA', seed=1, iteration_list=[200],
                  max_attempts=200, population_sizes=[70],
                  mutation_rates=[.2])
    ...
    for i in range(100, 1000, 100):
        ga = GARunner(problem=problem, experiment_name='GA', seed=i, iteration_list=[200],
                      max_attempts=200, population_sizes=[70],
                      mutation_rates=[.2])
    ...
    df_run_curves_ga[['Fitness', 'FEvals', 'Time']] = df_run_curves_ga[['Fitness', 'FEvals', 'Time']] / 5
    # df_run_curves_ga = df_run_curves_ga / 10
    fevals_ga = df_run_curves_ga['FEvals'].to_numpy()
    fit_ga = (df_run_curves_ga['Fitness'] * -1).to_numpy()
    clock_time_ga = sum(df_run_curves_ga['Time'].to_numpy())
    ...
    mimic = MIMICRunner(problem=problem, experiment_name='MIMIC', output_directory=None, seed=1,
                        iteration_list=[200], max_attempts=200, population_sizes=[700], keep_percent_list=[.4])
    ...
    for i in range(100, 1000, 100):
        mimic = MIMICRunner(problem=problem, experiment_name='MIMIC', output_directory=None, seed=i,
                           iteration_list=[200], max_attempts=200, population_sizes=[700],
                           keep_percent_list=[.4])
    ...
    df_run_curves_mimic[['Fitness', 'FEvals', 'Time']] = df_run_curves_mimic[['Fitness', 'FEvals', 'Time']] / 5
    # df_run_curves_ga = df_run_curves_ga / 10
    fevals_mimic = df_run_curves_mimic['FEvals'].to_numpy()
    fit_mimic = (df_run_curves_mimic['Fitness'] * -1).to_numpy()
    ...
    """
    plot_graph_fitness_iterations('Fitness vs Iteration: TSP using RHC vs SA vs GA vs MIMIC-Medium', array1=fit_rhc,
                                  array2=fit_sa, array3=fit_ga, array4=fit_mimic)
    plot_bar_max_fitness('Max Fitness: TSP using RHC vs SA vs GA vs MIMIC-Medium', array1=max(fit_rhc),
                         array2=max(fit_sa), array3=max(fit_ga), array4=max(fit_mimic))
    plot_bar_max_fitness_iteration('Max Fitness Iteration: TSP using RHC vs SA vs GA vs MIMIC-Medium',
                                   array1=fit_rhc,
                                   array2=fit_sa, array3=fit_ga, array4=fit_mimic)
    plot_graph_fevals_iterations('FEvals vs Iteration: TSP using RHC vs SA vs GA vs MIMIC-Medium', array1=fevals_rhc,
                                  array2=fevals_sa, array3=fevals_ga, array4=fevals_mimic)
    plot_clock_time_table('Clock Time: TSP using RHC vs SA vs GA vs MIMIC-Medium', array1=clock_time_rhc,
