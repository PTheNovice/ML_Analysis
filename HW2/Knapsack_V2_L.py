...
def plot_graph_fitness_iterations(title, array1, array2, array3, array4):
    ...
    plt.savefig('images/Knapsack_Fit_Large.png')
    plt.show()
...
def plot_bar_max_fitness(title, array1, array2, array3, array4):
    ...
    plt.legend()
    plt.savefig('images/Knapsack_Bar_Large.png')
    plt.show()
...
def plot_bar_max_fitness_iteration(title, array1, array2, array3, array4):
    ...
    plt.ylabel('Max Fitness')
    plt.legend()
    plt.savefig('images/Knapsack_MaxIteration_Large.png')
    plt.show()
...
def plot_graph_fevals_iterations(title, array1, array2, array3, array4):
    ...
    plt.savefig('images/Knapsack_FEvals_Large.png')
    plt.show()
...
def plot_clock_time_table(title, array1, array2, array3, array4):
    ...
    table.scale(1.2, 1.2)
    plt.savefig('images/Knapsack_ClockTime_Large.png')
    plt.show()
...
def test_knapsack():
    ...
    problem = KnapsackGenerator.generate(seed=7, number_of_items_types=6, max_item_count=40, max_weight_per_item=7,
                                         max_value_per_item=7, max_weight_pct=.8, multiply_by_max_item_count=True)

    """
    Randomized Hill Climbing (RHC) - Small, Need Med, Large
    """
    rhc = RHCRunner(problem=problem, experiment_name='RHC', output_directory=None, seed=1,
    ...
    """
    ga = GARunner(problem=problem, experiment_name='MaxK_GA', seed=1, iteration_list=[1, 10, 100, 1000],
    ...
    for i in range(100, 1000, 100):
        ga = GARunner(problem=problem, experiment_name='MaxK_GA', seed=i, iteration_list=[1, 10, 100, 1000],
                      max_attempts=1000, population_sizes=[50],
                      mutation_rates=[.47])
    ...
    mimic = MIMICRunner(problem=problem, experiment_name='RHC', output_directory=None, seed=1,
                        iteration_list=[1, 10, 100, 1000], max_attempts=1000, population_sizes=[1500], keep_percent_list=[.5])
    ...
    for i in range(100, 1000, 100):
        mimic = MIMICRunner(problem=problem, experiment_name='RHC', output_directory=None, seed=i,
                           iteration_list=[1, 10, 100, 1000], max_attempts=1000, population_sizes=[1500],
                           keep_percent_list=[.5])
    ...
    """
    plot_graph_fitness_iterations('Fitness vs Iteration: Knapsack using RHC vs SA vs GA vs MIMIC-Large', array1=fit_rhc,
                                  array2=fit_sa, array3=fit_ga, array4=fit_mimic)
    plot_bar_max_fitness('Max Fitness: Knapsack using RHC vs SA vs GA vs MIMIC-Large', array1=max(fit_rhc),
    ...
    plot_graph_fevals_iterations('FEvals vs Iteration: Knapsack using RHC vs SA vs GA vs MIMIC-Large', array1=fevals_rhc,
                                  array2=fevals_sa, array3=fevals_ga, array4=fevals_mimic)
    plot_clock_time_table('Clock Time: Knapsack using RHC vs SA vs GA vs MIMIC-Large', array1=clock_time_rhc,
