...
def test_car():
    ...
    # df_array = df.values
    # new_x = one_hot.fit_transform(df_array)

    # This is being used for A3
    x = df['class']
    new_df = df.drop(columns='class')
    encoded_df = pd.get_dummies(new_df)
    # print(encoded_df)
    trial_df = pd.concat([encoded_df, x], axis=1)
    trial_df['class'] = label_encoder.fit_transform(trial_df['class'])
    # print(trial_df)

    # This was used for A1 and A2
    # new_df = df
    ...
    trial_df.to_csv('data/FinalCar.csv', index=False)
